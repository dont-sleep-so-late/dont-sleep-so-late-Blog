...
```js
const [a, b, c] = 'strawberry'
console.log(a) // m
console.log(b) // a
console.log(c) // o

/* 解构 length 属性 */
const { length } = 'strawberry'
...
log('strawberry', 1, 2) // strawberry [1, 2]
log('strawberry', 1, 2, 3) // strawberry [1, 2, 3]
```
...
`indexOf()` 不够语义化，其含义是找到参数值的第一个出现位置，所以要去比较是否不等于 `-1`，表达起来不够直观
...
// 字符串会以字符数组的形式做合并
Object.assign({}, 'strawberry') // {0: 'm', 1: 'a', 2: 'o', 3: 'm', 4: 'a', 5: 'o'}
...
```js
const obj = { name: 'strawberry', age: 18 }
Object.keys(obj) // ['name', 'age']
Object.values(obj) //  ['strawberry', 18]
Object.entries(obj) // [['name', 'strawberry'], ['age', 18]]
```
...
```js
Object.fromEntries([['name', 'strawberry']]) // {name: 'strawberry'}

/* Map 转对象 */
const map = new Map([['name', 'strawberry']])
Object.fromEntries(map) // {name: 'strawberry'}

/* 将查询字符串转为对象 */
const params = 'name=strawberry&age=18'
Object.fromEntries(new URLSearchParams(params)) // {name: 'strawberry', age: '18'}
```
...
```js
/* ?? 运算符 */
const a = 0 ?? 'strawberry'         // 0
const b = '' ?? 'strawberry'        // ''
const c = null ?? 'strawberry'      // 'strawberry'
const d = undefined ?? 'strawberry' // 'strawberry'

/* || 运算符 */
const A = 0 || 'strawberry'         // 'strawberry'
const B = '' || 'strawberry'        // 'strawberry'
const C = null || 'strawberry'      // 'strawberry'
const D = undefined || 'strawberry' // 'strawberry'
```
...
- **`||` 运算符**只要左侧是 [假值](https://notes.fe-mm.com/fe/javascript/conversions#toboolean) 就会返回右侧的值
...
> 相关文章
>
> - [精读《默认、命名导出的区别》](https://github.com/ascoders/weekly/blob/master/前沿技术/204.精读《默认、命名导出的区别》.md)
...
[Promise 实现](https://github.com/strawberry2002/Promise) 简易实现、A+ 规范实现、原型方法、静态方法实现