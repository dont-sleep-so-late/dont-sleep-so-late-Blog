...
[查看完整版 Git 命令](https://dont-sleep-so-late.github.io/2023/09/06/git/)
...
1. 添加远程仓库 `daily-notes`（`remote` 取名为 `daily-notes`）

```sh
git remote add daily-notes http://github.com/maomao1996/daily-notes
```

1. 拉取 `daily-notes` 的 `master` 分支到本地（本地分支名为 `notes`）
...
1. 关联分支（本地分支 `notes` 关联远程仓库 `daily-notes` 的 `master` 分支）
